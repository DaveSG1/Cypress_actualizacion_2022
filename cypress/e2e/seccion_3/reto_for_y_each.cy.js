...
describe("Reto bucles For y each", () => {
    ...
    it("Reto bucles", () => {  
        ...
        for(let x=0; x<=3; x++){
            ...
            cy.wait(1500)
            cy.get('.col-sm-12 > .btn').click() //le digo que, abierta la ventana del móvil que toque en esa vuelta, le de al botón de añadir a la cesta
            cy.wait(1500) 
            cy.get('.active > .nav-link').click() // le digo que, una vez añadido a la cesta, haga click en el botón Home
            cy.wait(1500) 
        }

        cy.get(':nth-child(4) > .nav-link').click() // le digo que, una vez terminado el bucle y añadidos todos los móviles a la cesta, haga click en la cesta para comprobar que todos los móviles indicados (4) se han añadido
